...
export default function PdfViewerWithUpload({ knowledgeBases, loading, selectedKnowledgeBaseId, targetPage, highlightText, onPageChanged }: PdfViewerWithUploadProps) {
  ...
  // Highlight text after page is rendered (fuzzy token-based)
  useEffect(() => {
      ...
      try {
        setTimeout(() => {
          ...
          if (!bestRange || bestScore <= 0) {
            // Nothing found on this page – do not show the old exact-match warning
          ...
          // If nothing within window matched at token level, still softly highlight the window for context
          ...
          if (firstHighlighted) {
            ...
            // Clear params after a short delay so repeated jumps re-run
