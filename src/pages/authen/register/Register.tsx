...
const Register: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // Backend expects nested payload { user: { username, email, password }, person: { firstName, lastName } }
      const payload = {
        user: {
      ...
      // Use the generic register helper which posts to /api/auth/register by default.
      // The backend example you provided uses POST /api/users - call that directly via fetch so we match the expected shape.
      ...
      try {
        ...
        console.debug("Register: non-json response", text);
        data = text;
      }

      if (res.ok && data && (data.code === 1000 || data.code === "1000")) {
        // Registration succeeded. Navigate to login and show a brief success message.
        navigate("/login", { replace: true });
      } else {
        const msg =
          (data && (data.message || data.error)) ||
          `Lỗi khi đăng ký (${res.status})`;
        setError(msg);
      }
    } catch (err: any) {
