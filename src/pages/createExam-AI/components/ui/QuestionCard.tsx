        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-gray-500 text-xs">
              Độ khó:{" "}
              {question.hardLevel === 1
                ? "Dễ"
                : question.hardLevel === 2
                ? "Trung bình"
                : "Khó"}
            </span>

            {/* ✅ DI CHUYỂN BADGES LÊN ĐÂY - CHỈ HIỂN THỊ 1 LẦN */}
            {hasMathContent(question.content) && (
              <Tag color="purple">Có công thức</Tag>
            )}
            {hasMarkdownTable(question.content) && (
              <Tag color="cyan">Có bảng dữ liệu</Tag>
            )}
          </div>
              ...
              {question.answers.map((answer, idx) => (
                <div
                  ...
                  onClick={() =>
                    ...
                    onToggleCorrect &&
                    onToggleCorrect(question.id, idx)
                  }
                >
                  ...
                  <MathContent
                    ...
                    className={`flex-1 text-gray-700 ${
                      answer.isCorrect ? "font-bold text-green-700" : ""
                    }`}
                  />
                  <Tooltip
                    title={
            ...
            {question.hasImage || question.questionImg || question.imageSrc ? (
              <div className="w-full border border-gray-200 rounded-lg p-4 bg-gray-50 flex items-center justify-center relative">
                <div className="text-center object-cover">
                  <Image
                    src={question.questionImg || question.imageSrc || mockImage}
                    alt="Question image"
                  />
              ...
              <div className="border border-gray-200 rounded-lg p-4 bg-gray-50 h-64 flex flex-col items-center justify-center gap-3">
                ...
                <ImageUploadOnly
                  onImageUploaded={(imageUrl) =>
                    question.id !== undefined &&
                    onImageUpload &&
                    onImageUpload(question.id, imageUrl)
                  }
                />
              </div>
            )}
